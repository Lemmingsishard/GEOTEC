...
#musik thigns
pygame.mixer.init()
jinput = open(f"{figgy[0]}/sound.json", "r")
musikdata = json.load(jinput)
game_song = musikdata["game_song"]
title_song = musikdata["title_song"]
enabled = musikdata["Enabled"]
ontitle = True
...
teYt = []
tetagst = []
sound_rect = pygame.rect.Rect(0, 0, 0, 0)
...
while True:
            ...
            if event.button == 1:
                if sound_rect.collidepoint((mouse_pos)):
                    if enabled == True:
                        ...
                        enabled = True
                    musikdata["Enabled"] = enabled
                    with open(f"{figgy[0]}/sound.json", "w") as module:
                        json.dump(musikdata, module, indent=4)
                        module.close()

                    
                if Edit_Mode == True and title == 0:
    ...
    if title == 1:
        ...
        ontitle = True
        if enabled == True:
            sound = pygame.image.load("assets/aero_sound.png")
            sound_rect = sound.get_rect(topleft = (590, 430))
            screen.blit(sound, sound_rect)
        else:
            sound = pygame.image.load("assets/aero_orb.png")
            sound_rect = sound.get_rect(topleft = (590, 430))
            screen.blit(sound, sound_rect)

    #allows for game music
    if title == 0:
        musik = Engine_Sound.GeoPlayer(ontitle, enabled, game_song)
    if title == 1:
        title_musik = Engine_Sound.TitlePlayer(ontitle, enabled, title_song)
